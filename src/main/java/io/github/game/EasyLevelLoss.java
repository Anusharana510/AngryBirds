...
public class EasyLevelLoss implements Screen {
    private Stage stage_;
    private SpriteBatch batch;
    private Skin skin1;
    private Texture background;
    private TextButton Repeat_Level;
    private TextButton Quit;
    private BitmapFont font;

    private AngryBirds game;
    public EasyLevelLoss(AngryBirds game){this.game = game;
    }
    ...
    public void show() {
        ...
        Repeat_Level = new TextButton("Repeat Level",skin1);
        Repeat_Level.addListener(new ClickListener(){
            public void clicked(InputEvent event, float x, float y){
                game.setScreen(new EasyLevel(game));
            }
        });
        Repeat_Level.setPosition(250,270);
        Repeat_Level.setSize(100,100);
        stage_.addActor(Repeat_Level);
