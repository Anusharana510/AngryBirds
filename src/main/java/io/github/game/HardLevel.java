...
public class HardLevel implements Screen {
    ...
    public void show() {
        ...
        background = new Texture("easybg.jpg");
        ...
        pig = new Texture("pig_failed.png");
        pig1 = new Texture("pig.png");
        ...
        float w_ = 1.0f;
        float h_ = 3.0f;
        setting_blocks(10,2,w_,h_,rock_block);
        setting_blocks(10,9,w_,h_,block1);
        setting_blocks(12,2,w_,h_,block1);
        setting_blocks(12,9,w_,h_,glass_block);
...
//        setting_blocks(8,1,1,1);
    ...
    public void render(float delta) {
        ...
        for (Body pigBody : pigs) {
            ...
            if (pigPosition.x < -2 || pigPosition.x > 20 || pigPosition.y < -2 || pigPosition.y > 13) {
                destroy_these_bodies.add(pigBody);
                score += 10;
                remaining_pigs--;

                if (remaining_pigs == 0) {
                    game.setScreen(new HardLevelWin(game));
                }
            }
        }
        for (Body body : destroy_these_bodies) {
    ...
    void loss (){
        if (!bird_in_motion && remaining_pigs>0 && attempt==0){
            game.setScreen(new HardLevelLoss(game));
        }
...
//                System.out.println("Collision detected between: " + fixtureA + " and " + fixtureB);
                ...
                if ((first_body == pig && Second_body == "Ground") || (first_body == "Ground" && Second_body == pig)
                    ...
                    || (first_body == pig2 && Second_body == "Ground") || (first_body == "Ground" && Second_body == pig2)) {
                    for (Body p_body : pigs) {
                        if (p_body == f1.getBody() || p_body == f2.getBody()) {
                            ...
                            if (remaining_pigs == 0){
                                game.setScreen(new HardLevelWin
                                    (game));
                            }
                        }
                    }
